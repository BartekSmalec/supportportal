...
class JwtTokenProviderTest {
    ...
    void setUp() {
        one = User.builder().userId("2131j2asda").email("one@gmail.com").firstName("Joe").lastName("Doe")
                .username("jdoe").isActive(true).isNonLocked(true).authorities(authorities_user)
                .password(new BCryptPasswordEncoder().encode("somepass")).build();

        userPrincipal = new UserPrincipal(one);
        token = jwtTokenProvider.generateToken(userPrincipal);

        expiredToken = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJhdWQiOiJVc2VyIE1hbmFnZW1lbnQgUG9ydGFsIiwic3ViIjoiaW5mYW1pYSIsImlzcyI6IlN1cHBvcnQgQXBwLCBMTEMiLCJBdXRob3JpdGllcyI6WyJ1c2VyOnJlYWQiXSwiZXhwIjoxNjc3MDcwOTYyLCJpYXQiOjE2NzY2Mzg5NjJ9.L-Z6_2-xA3xSN6E3WA1SykKepZGlcIgHdoQfvzbMIuSDiPr1cNagZoLP92iv1cSeggeB4RokP2g_3Yen39EVtA";
    }

    @Test
    void testIsTokenValid() {

        assertThrows(TokenExpiredException.class, () -> jwtTokenProvider.isTokenValid(username, expiredToken));
    }
